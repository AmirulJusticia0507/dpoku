...
// Riwayat status
$logStmt = $koneksidpogendeng->prepare(
    "SELECT sl.*, u.username FROM dpo_status_log sl
     LEFT JOIN \"user\" u ON u.id = sl.changed_by
     WHERE sl.dpo_id = ? ORDER BY sl.id DESC");
$logStmt->execute([$id]);
        ...
        <ol class="relative border-l border-gray-200 ml-3">
          <?php foreach ($statusLogs as $s): ?>
            <li class="mb-4 ml-6">
              ...
              <p class="text-xs text-gray-400"><?= htmlspecialchars($s['created_at']) ?> (oleh: <?= htmlspecialchars($s['username'] ?? ('user id ' . (int) $s['changed_by'])) ?>)</p>
            </li>
          <?php endforeach; ?>
        </ol>
      <?php else: ?>
